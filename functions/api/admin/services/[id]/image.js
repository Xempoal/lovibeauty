// POST /api/admin/services/:id/image  →  upload a new photo for a service
//   body: { data: <base64, no data-URL prefix>, contentType: 'image/jpeg'|'image/png'|'image/webp' }
//
// Uploads to the service-images bucket, points services.image_url at the new
// public URL, and deletes the previous object if it lived in our bucket.
...
export async function onRequestPost({ request, env, params }) {
  ...
  // Make sure the service exists and grab its current image_url.
  let current;
  try {
    const rows = await sb(env, 'services?select=id,image_url&id=eq.' + encodeURIComponent(id));
  ...
  const path = 'services/' + id + '-' + Date.now() + '.' + EXT[contentType];
  ...
  try {
    ...
    return json({ service: rows && rows[0], image_url: publicUrl });
  } catch (err) {
    // Upload succeeded but the DB update failed — roll back the orphan object.
